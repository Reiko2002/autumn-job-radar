...
def merge(previous_jobs,new_jobs,successful,now):
...
async def main():
    ...
    jobs=merge(previous['jobs'],deduplicate(normalized),successful,started)
    ...
    payload={'schemaVersion':1,'generatedAt':started.isoformat(),'stats':stats,'jobs':jobs}; errors=run_quality_gate(previous,payload)
