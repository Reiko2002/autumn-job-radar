...
export default function App() {
  ...
  return <Shell page={page} setPage={navigate} savedCount={savedIds.size}>
    ...
    {!loading && !loadError && (page === 'jobs' || page === 'saved') && <><div className="sync-banner">● {syncInfo} · 岗位信息以企业官方页面为准</div><JobsPage title={page === 'jobs' ? '找到真正适合你的机会' : '我的收藏'} subtitle={page === 'jobs' ? '来自公开招聘源，每个推荐都有可解释理由。' : '把值得认真研究的岗位放在同一个地方。'} jobs={visibleJobs} allCount={filterJobs(jobs, matches, profile, initialFilters).length} matches={matches} filters={filters} setFilters={setFilters} userJobs={userJobs} selected={selected} setSelected={setSelected} onSave={onSave} onStatus={onStatus} onPreferences={() => navigate('preferences')} savedView={page === 'saved'} /></>}
    {!loading && !loadError && page === 'pipeline' && <><PipelinePage jobs={jobs} matches={matches} userJobs={userJobs} onStatus={onStatus} onSelect={setSelected} />{selected && <div className="detail-overlay"><JobDetail job={selected} match={matches[selected.id]} state={userJobs[selected.id]} onSave={() => onSave(selected.id)} onStatus={(status) => onStatus(selected.id, status)} onClose={() => setSelected(undefined)} /></div>}</>}
