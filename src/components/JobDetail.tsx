...
export function JobDetail({ job, match, state, onSave, onStatus, onClose }: { job: Job; match: MatchResult; state?: UserJobState; onSave: () => void; onStatus: (status: ApplicationStatus) => void; onClose: () => void }) {
  return <aside className="detail-panel" aria-label="岗位详情">
    ...
    <section className="detail-section"><h3>我们期待</h3><ul>{job.requirements.map((item) => <li key={item}>{item}</li>)}</ul></section>
    <section className="status-section"><label htmlFor="detail-status"><BriefcaseBusiness size={16} />投递状态</label><select id="detail-status" value={state?.applicationStatus ?? ''} onChange={(event) => onStatus(event.target.value as ApplicationStatus)}><option value="" disabled>选择状态</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></section>
  </aside>
}
