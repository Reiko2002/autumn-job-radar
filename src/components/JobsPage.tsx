...
export function JobsPage({ title, subtitle, jobs, allCount, matches, filters, setFilters, userJobs, selected, setSelected, onSave, onStatus, onPreferences, savedView = false }: { title: string; subtitle: string; jobs: Job[]; allCount: number; matches: Record<string, MatchResult>; filters: Filters; setFilters: (filters: Filters) => void; userJobs: Record<string, UserJobState>; selected?: Job; setSelected: (job?: Job) => void; onSave: (id: string) => void; onStatus: (id: string, status: ApplicationStatus) => void; onPreferences: () => void; savedView?: boolean }) {
  ...
  return <div className={`jobs-layout ${selected ? 'with-detail' : ''}`}>
    <div className="jobs-content">
      ...
      <div className="job-list">{jobs.map((job) => <JobCard key={job.id} job={job} match={matches[job.id]} state={userJobs[job.id]} selected={selected?.id === job.id} onSelect={() => setSelected(job)} onSave={() => onSave(job.id)} onPipeline={() => onStatus(job.id, userJobs[job.id]?.applicationStatus ?? 'wishlist')} />)}{jobs.length === 0 && <div className="empty-state"><div>⌁</div><h2>{savedView ? '还没有收藏岗位' : '没有找到匹配岗位'}</h2><p>{savedView ? '在岗位广场遇到心仪机会时，点一下收藏即可。' : '尝试减少筛选条件，或换一个关键词。'}</p><button className="primary-button" onClick={clear}>{savedView ? '查看岗位广场' : '清除全部筛选'}</button></div>}</div>
    </div>
    {selected && <JobDetail job={selected} match={matches[selected.id]} state={userJobs[selected.id]} onSave={() => onSave(selected.id)} onStatus={(status) => onStatus(selected.id, status)} onClose={() => setSelected()} />}
  </div>
}
